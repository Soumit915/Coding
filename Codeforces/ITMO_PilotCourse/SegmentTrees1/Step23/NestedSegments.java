    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int i=0;i<n;i++){
             if(segments[arr[i]-1].start!=-1)
                 segments[arr[i]-1].end = i;
             else segments[arr[i]-1].start = i;
        }
        ...
        for(int i=0;i<n;i++){
            if(segments[arr[i]-1].end == i){
                int q = query(segTree, 0, 0, n-1, segments[arr[i]-1].start,
                        segments[arr[i]-1].end);
                ans[segments[arr[i]-1].id] = q;
                update(segTree, 0, segments[arr[i]-1].start, 1, 0, n-1);
            }
        }
