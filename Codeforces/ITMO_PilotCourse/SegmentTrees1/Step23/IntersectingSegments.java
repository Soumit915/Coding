...
public class IntersectingSegments {
    ...
    static int query(int[] segTree, int si, int ll, int ul, int start, int end){
        //no overlap
        if(end<ll || start>ul)
            return 0;
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int i=0;i<n;i++){
            if(segments[arr[i]-1].start==-1)
                segments[arr[i]-1].start = i;
            else segments[arr[i]-1].end = i;
        }
        ...
        for(int i=0;i<n;i++){
            if(segments[arr[i]-1].end==i){
                update(segTree, 0, segments[arr[i]-1].start, 0, 0, n-1);
                int q = query(segTree, 0, 0, n-1,
                        segments[arr[i]-1].start, segments[arr[i]-1].end);
                ans[arr[i]-1] = q;
            }
            else{
        ...
        segTree = new int[sn];
        for(int i=n-1;i>=0;i--){
            if(segments[arr[i]-1].start==i){
                update(segTree, 0, segments[arr[i]-1].end, 0, 0, n-1);
                int q = query(segTree, 0, 0, n-1,
                        segments[arr[i]-1].start, segments[arr[i]-1].end);
                ans[arr[i]-1] += q;
            }
            else update(segTree, 0, i, 1, 0, n-1);
        }
