...
public class CheckConsistencyOfCSCurriculam {
    static class Node{
        int id;
        boolean isVisited;
    ...
    static class Graph{
        ...
        {
            this.nodelist.get(u).adlist.add(this.nodelist.get(v));
        }
        boolean hasCycle(){
            for(Node nd: nodelist)
                if(!nd.isVisited && dfs(nd)) {
                    return true;
                }
            return false;
        }
        boolean dfs(Node nd)
        {
            ...
            while(!stk.isEmpty())
            {
                ...
                if(ptr<cur.adlist.size()-1)
                {
                    ...
                    if(next.beingVisited) {
                        return true;
                    }
                    else if(!next.isVisited){
                        stk.push(next);
                        ptrstk.push(-1);
                        next.beingVisited = true;
                    }
                }
                else{
                    cur.isVisited = true;
                    cur.beingVisited = false;
                }
            }
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println(gr.hasCycle()?1:0);
