...
public class A1 {
    public static void main(String[] args) throws IOException {
        ...
        for(int testi = 1;testi<=t;testi++){
            ...
            HashSet<Integer> vowelset = new HashSet<>();
            vowelset.add(('A'-65));
            vowelset.add(('E'-65));
            vowelset.add(('I'-65));
            vowelset.add(('O'-65));
            ...
            for(int i=0;i<26;i++){
                if(vowelset.contains(i))
                    vowelcount += hash[i];
                else consonentcount += hash[i];
            }

            int min = Integer.MAX_VALUE;
            for(int i=0;i<26;i++){
                if(vowelset.contains(i)){
                    min = Math.min(min, consonentcount+(vowelcount-hash[i])*2);
                }
                else{
                    min = Math.min(min, vowelcount+(consonentcount-hash[i])*2);
                }
            }
