        ...
        try (Scanner sc = new Scanner(file, StandardCharsets.UTF_8.name())) {
            ...
            for(int testi=1;testi<=t;testi++)
            {
                ...
                for(int i=1;i<n;i++)
                {
                    ...
                    final long extraperimeter1 = 2 * (w[i] + h[i]);
                    ...
                    else if(previous.left<=current.left && previous.right<=current.right)
                    {
                        ...
                        if(next==null || next.left>current.right)
                        {
                            extraperimeter += 2*(current.right-previous.right);
                            extraperimeter %= mod;
                        ...
                        else
                        {
                            while(next!=null && next.left<=current.right)
                            {
                                extraperimeter += 2*(next.left-previous.right);
                                extraperimeter -= (2*h[i])%mod;
                                extraperimeter %= mod;
                                previous = next;
                                next = intervalTree.higher(previous);
                                intervalTree.remove(previous);
                    ...
                    {
                        start = current.left;
                        extraperimeter += 2*(previous.left-current.left);
                        extraperimeter %= mod;
                        intervalTree.remove(previous);
                    ...
                    {
                        extraperimeter += (previous.left-current.left)*2;
                        extraperimeter %= mod;
                        intervalTree.remove(previous);
