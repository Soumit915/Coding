...
public class FactorialProblem {
    ...
    static int getPowers(long n, long k){
        int count = 0;
        long kc = k;
        while(kc<=n){
            count += n/kc;
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-->0){
            ...
            ArrayList<Integer> pfactors = getFactors(k);
            HashMap<Integer, Integer> factor_hash = new HashMap<>();
            for(int i: pfactors){
                factor_hash.put(i, factor_hash.getOrDefault(i, 0)+1);
            }

            int x = Integer.MAX_VALUE;
            for(int i: factor_hash.keySet()){
                x = Math.min(getPowers(n, i)/factor_hash.get(i), x);
            }
