    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-->0)
        {
            ...
            for(int i=0;i<n;i++)
            {
                ...
                while(val>1)
                {
                    int count = hash2.getOrDefault(spf.get(val), 0);
                    count++;
                    hash2.put(spf.get(val), count);
                    val /= spf.get(val);
                }
            }
            ...
            for(int i=0;i<n-1;i++)
            {
                ...
                while(val>1)
                {
                    ...
                    int count1 = hash1.getOrDefault(spf.get(val), 0);
                    count1++;
                    hash1.put(spf.get(val), count1);
                    val /= spf.get(val);
                }

                boolean flag = true;
                for(int j: hash1.keySet())
                    if(hash2.containsKey(j) && hash2.get(j)!=0)
                    {
                        flag = false;
                        break;
                    }
