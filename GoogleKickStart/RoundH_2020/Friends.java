...
public class Friends {
    public static boolean isFriend(String a, String b)
    {
        ...
        {
            hash[a.charAt(i)-65] = true;
        }

        boolean[] hash1 = new boolean[26];
        for(int i=0;i<b.length();i++)
            hash1[b.charAt(i)-65] = true;
        ...
        boolean flag = false;
        for (int i=0;i<hash.length;i++) flag |= (hash[i]&hash1[i]);
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        for(int testi=1;testi<=t;testi++)
        {
            ...
            for(int i=1;i<n;i++)
            {
                if(i==1 && isFriend(names[i-1], names[i]))
                {
                    friend[0] = 1;
                    friend[1] = 1;
                }
                else if(isFriend(names[i-1], names[i]))
                    friend[i] = 1;
            }

            for(int i=1;i<n;i++)
                friend[i] = friend[i-1]+friend[i];
