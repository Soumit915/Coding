...
public class C1 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int testi = 1;testi<=tc;testi++){
            ...
            long ans = 0;
            for(int i=0;i<lim;i++){
                List<Vegetable> vege_list = new ArrayList<>();
                for(int j=0;j<n;j++){
                    if(isSet(i, j)){
                        vege_list.add(vegetables[j]);
                    }
                }

                vege_list.sort((v1, v2) -> Long.signum(v2.li - v1.li));
                boolean flag = true;
                for(int j=0;j<vege_list.size();j++){
                    if(j + vege_list.get(j).li >= d){
                        flag = false;
                        break;
                    }
                }
                ...
                long cost = 0;
                if(flag){
                    for(Vegetable vegetable : vege_list){
                        cost += vegetable.vi;
                    }
                }
